...
def front():
    ...
    global pub,i,erry,errx

    v = Twist()
    v.linear.x = unitDistancex + erry
    v.linear.y,v.linear.z  = -errx,0.0
    ...
    erry = y - (0.09*(2*(7-i) + 1))  #erry is positive => robot is a little bit more upwards thus it should go more front
    errx = x - (0.09*(2*(j-7) - 1))  #errx is positive => robot is a little bit towards right thus it should go towards left

    v.linear.y,v.linear.x,v.linear.z = 0.0,0.0,0.0
    pub.publish(v)

    print("erry: ",erry," errx: ",errx)
...
def left():
    ...
    global pub,j,errx,erry

    #print("---------------- inside left -------------------------")

    v = Twist()
    v.linear.y = -(unitDistancey + errx)
    v.linear.x,v.linear.z  = erry,0.0
    v.angular.z,v.angular.x,v.angular.y = 0.0,0.0,0.0
...
def main():
    ...
    rospy.init_node('error_control', anonymous=True)
